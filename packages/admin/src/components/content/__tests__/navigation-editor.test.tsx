...
describe('NavigationEditor', () => {
  ...
  it('renders existing children as display rows', () => {
    ...
    expect(screen.getByTestId('nav-child-0-0')).toBeInTheDocument();
    expect(screen.getByTestId('nav-child-label-0-0')).toHaveTextContent('Widget');
    expect(screen.getByTestId('nav-child-href-0-0')).toHaveTextContent('/products/widget');
  });
  ...
  it('removes a child item', async () => {
    ...
    expect(screen.getByTestId('nav-child-label-0-0')).toHaveTextContent('Gadget');
  });
  ...
  it('edits a child item via editor panel', async () => {
    ...
    expect(screen.getByTestId('nav-editor-href')).toHaveValue('/products/widget');
    ...
    expect(screen.getByTestId('nav-child-label-0-0')).toHaveTextContent('Gizmo');
  });
  ...
  it('saves edited values through onSave after apply', async () => {
    ...
    expect(handleSave).toHaveBeenCalledWith([{ label: 'About', href: '/', children: [] }]);
  });
  ...
  it('auto-applies dirty edits when selecting a different item', async () => {
    ...
    expect(screen.getByTestId('nav-item-label-0')).toHaveTextContent('Homepage');

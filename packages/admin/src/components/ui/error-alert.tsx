  ...
  if (variant === 'card') {
    return (
      ...
      >
        ...
        {title && <p className="text-[16px] font-semibold text-[var(--admin-gray-900)]">{title}</p>}
        <p className="text-[14px] text-[var(--admin-gray-500)] mt-1 max-w-sm">{children}</p>
        {onRetry && (
          <Button
            type="button"
            variant="secondary"
  ...
  if (onRetry) {
    return (
      <div className={cn(className)} data-testid={testId} role="alert">
        {title && <p className="text-sm font-semibold text-[var(--admin-error-700)]">{title}</p>}

...
| C8 | Field type: `number` | `missing` | `fields.ts:60-140` (absent) | No numeric field; roadmap-flagged (`roadmap.md:27`). Editors cannot model numbers without raw zod (losing the admin field-type) |
...
| C11 | `core` depends only on `zod` | `ok` | `index.ts:1-30`, all `src/*` imports | Constitution boundary respected (`constitution.md`); no `any` in the surface read |
...
- **R1 — `createSectionRenderer` calls components as plain functions
  ...
  outside React's reconciler — hooks throw or misbehave. The README claims
  "Framework-agnostic: works with React, Preact, Vue" (`section-renderer.ts:5`),
  which holds only for stateless components. This matches the project's recorded
...
- **C8/C9 (number, date)** are the headline core capability gaps. They are not
  ...
  `drift`; but for a content CMS their absence is material and the roadmap already
  anticipates them. Remediation routing (new field types -> `@structcms/core` per
  `architecture.md:32`) is for the synthesis issue (#27) to schedule.
